...
function ChallengeCard({ lesson }) {
  return (
    <li className="group relative transition hover:-translate-y-0.5">
      <Ribbon color={lesson.language === 'HTML' ? 'fuchsia' : 'emerald'} position="top-right">
        {lesson.language}
      </Ribbon>

      <div className="w-full overflow-hidden rounded-lg bg-white shadow-md transition group-hover:shadow-xl">
        <img src="/images/thumbnails/ribbon.jpg" alt="" />
        <h2 className="border-t border-slate-200 p-8 text-center text-2xl font-semibold text-gray-900">
          {lesson.name}
        </h2>
      </div>
